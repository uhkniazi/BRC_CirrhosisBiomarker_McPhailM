...
parameters {
  ...
  real<lower=0> tau[iMixtures]; // standard deviations for deflections
}
transformed parameters {
  vector[Ntotal] muMix1; // number of fitted values
  vector[Ntotal] muMix2; // number of fitted values
  vector[Ncol1] betasMix1_2; // regression parameters for each mixture component
  vector[Ncol2] betasMix2_2; // regression parameters for each mixture component
  // calculate fitted values without intercept
  betasMix1_2 = betasMix1 * tau[1];
  betasMix2_2 = betasMix2 * tau[2];
  muMix1 = X1 * betasMix1_2;
  muMix2 = X2 * betasMix2_2;
  //mu = inv_logit(mu);
}
model {
  ...
  mu[1] ~ cauchy(0, 2.5);
  mu[2] ~ cauchy(0, 2.5);
  betasMix1 ~ normal(0, 1);
  betasMix2 ~ normal(0, 1);
  tau ~ cauchy(0, 1);
